...
(function () {
  ...
  /* ================================================================
   * PROMO / HARGA RESELLER
   * Menggunakan tabel promos (type='Diskon') yang sudah ada di sistem.
   * ================================================================ */
  function _resellerPromos() {
    return (SS.DB.promos || []).filter(function(p){ return p.type === 'Diskon'; });
  }
  // Hitung harga setelah diskon promo. Value: "15%" atau "40000" atau "Rp40.000"
  function _calcResellerPrice(hargaNormal, promoValue) {
    var v = (promoValue || '').trim();
    if (!v) return hargaNormal;
    if (v.indexOf('%') >= 0) {
      var pct = parseFloat(v) || 0;
      return Math.max(0, Math.round(hargaNormal * (1 - pct / 100)));
    }
    var amt = parseInt(v.replace(/\D/g, ''), 10) || 0;
    return Math.max(0, hargaNormal - amt);
  }
  ...
  function _getEditedPrice() {
    ...
    if (type === 'reseller') {
      var promoId = SS.S.k_editResellerPromoId;
      var promo   = _resellerPromos().find(function(p){ return p.id === promoId; });
      if (!promo) return item.harga;
      return _calcResellerPrice(item.harga, promo.value);
    }
    // custom
  ...
  function _editPriceModalHtml() {
    ...
    var selPromoId = SS.S.k_editResellerPromoId;
    var promos     = _resellerPromos();
    var preview    = _getEditedPrice();
    var hasChanged = preview !== item.harga;

    function _tabBtn(k, lbl) {
      var on = type === k;
      return '<button '+SS.A(function(){ SS.setState({k_editType:k,k_editResellerPromoId:null,k_editCustom:''}); })
    ...
    // Panel Harga Reseller
    var resellerPanel = '';
    if (type === 'reseller') {
      if (promos.length === 0) {
        resellerPanel = '<div style="margin-bottom:13px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px 14px;font-size:12.5px;color:var(--muted);">'
          +'Belum ada promo Diskon.<br>Tambahkan di Dashboard Admin &rarr; Promo &amp; Bundle.'
          +'</div>';
      } else {
        resellerPanel = '<div style="margin-bottom:13px;">'
          +_lbl('Pilih Template Diskon')
          +'<div style="display:flex;flex-direction:column;gap:7px;">'
          +promos.map(function(pr){
            var on         = selPromoId === pr.id;
            var prPreview  = _calcResellerPrice(item.harga, pr.value);
            return '<button '+SS.A(function(){ SS.setState({k_editResellerPromoId:pr.id}); })
              +' style="text-align:left;padding:10px 13px;border-radius:11px;cursor:pointer;'
              +'border:1.5px solid '+(on?'var(--gold)':'var(--border)')+';'
              +'background:'+(on?'var(--goldtint)':'var(--surface2)')+';">'
              +'<div style="font-size:13px;font-weight:700;color:'+(on?'var(--gold)':'var(--text)')+';">'
                +SS.esc(pr.name)
                +' <span style="font-size:11px;font-weight:500;color:var(--muted);">('+SS.esc(pr.value)+')</span>'
              +'</div>'
              +'<div style="font-size:11.5px;color:var(--muted);margin-top:2px;">'
                +SS.rp(item.harga)+' &rarr; '
                +'<span style="color:'+(on?'var(--gold)':'var(--ok)')+';">'+SS.rp(prPreview)+'</span>'
              +'</div>'
              +'</button>';
          }).join('')
          +'</div></div>';
    ...
    return '<div '+SS.A(function(){ SS.setState({k_editIdx:null}); })
      ...
      +'<div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:201;'
        ...
        +'<div style="display:flex;gap:6px;margin-bottom:14px;">'
          ...
          +_tabBtn('reseller','Harga Reseller')
