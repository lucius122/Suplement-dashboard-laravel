...
(function () {
    ...
    async function loadAll() {
        ...
        DB.yearly = {};
        DB.expenses = [];
        DB.expenseCategories = []; // data berubah setelah tulis → buang cache
        if (USER && USER.role === "admin") {
            await loadByUser(S.uPeriod);
            if (S.period === "Bulanan")
                await loadYearly(new Date().getFullYear());
    ...
    let S = {
        ...
        role: null,
        who: "",
        branch: "Pleburan", // 'boot' = latar kosong selama cek sesi
        ...
        memberOpen: null,
        memberSearch: "",
        memberDropdown: false, // selMembers = pegawai dipilih utk banding ([] = semua)
        uName: "",
        uUname: "",
        uPass: "",
        uRole: "Kasir",
        uCabang: "Pleburan",
        editUserId: null, // null = mode tambah
        pName: "",
        pVar: "",
        pKat: "",
        pHarga: "",
        ...
        pBarcode: "",
        pExp: "",
        pBranch: "", // form tambah produk (admin)
        poForm: false,
        poName: "",
        poAmount: "",
        poDue: "", // form Purchase Order (hutang supplier)
        promoForm: false,
        prName: "",
        prDesc: "",
        prType: "Bundle",
        prValue: "", // form promo/bundle
        restockId: null,
        restockQty: "", // modal tambah stok (null = tutup)
        confirmPay: null, // modal konfirmasi tandai lunas: {kind:'receivable'|'supplier', row} | null
        scanTarget: "stok",
        scanManual: "",
        scanMsg: "", // scan barcode (kamera + input manual)
        scanDevices: [],
        scanDeviceId: null, // daftar kamera terdeteksi (mis. DroidCam) + pilihan aktif
        biayaForm: false,
        bxCategory: "Sewa",
        bxNote: "",
        bxAmount: "",
        ...
        bxRecurring: false,
        bxDueDay: "",
        bxDate: "", // form Biaya Operasional
        bxCatForm: false,
        newBxCat: "", // modal Kelola Kategori Biaya
        editExpenseId: null, // null = form Catat Biaya mode tambah, else = id yg sedang diedit
        editBxCatId: null, // null = mode tambah kategori, else = id kategori yg sedang di-rename
        ...
        k_scanManual: "",
        k_restockId: null,
        k_restockQty: "", // modal restock setelah scan masuk
        theme: "dark",
    ...
    const fmtDate = (s) => {
        ...
        return isNaN(date.getTime())
            ? String(s)
            : date.getDate() + " " + MON[date.getMonth()];
    };
    const daysLeft = (s) => {
        if (!s) return 0;
    ...
    function ic(name, color, size) {
        ...
        const P = {
            ...
            piutang: [
                "M6 3h12v18l-2-1.3-2 1.3-2-1.3-2 1.3-2-1.3V3Z",
                "M9 8h6M9 12h5",
            ],
            tempo: ["M12 7v5l3 2"],
            stok: ["M21 8l-9-5-9 5 9 5 9-5ZM3 8v8l9 5 9-5V8M12 13v8"],
            produk: [
                "M20.6 12.6 12 4H5v7l8.6 8.6a1.4 1.4 0 0 0 2 0l4-4a1.4 1.4 0 0 0 0-2Z",
                "M8 8h.01",
            ],
            laporan: ["M4 20h16", "M4 14l5-5 4 4 6-7"],
            users: [
                "M17 20a5 5 0 0 0-10 0",
                "M12 11a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4Z",
            ],
            supplier: [
                "M3 6h11v10H3zM14 9h4l3 3v4h-7",
                "M7.5 18.5a1.6 1.6 0 1 0 .01 0M16.5 18.5a1.6 1.6 0 1 0 .01 0",
            ],
            promo: [
                "M20 12v8H4v-8M2 7.5h20V12H2zM12 7.5V20M12 7.5C11 4 9 3 7.5 3.5S5.5 7 8 7.5M12 7.5C13 4 15 3 16.5 3.5S18.5 7 16 7.5",
            ],
            shopee: [
                "M5 8h14l-1 12a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1L5 8Z",
                "M9 8V6a3 3 0 0 1 6 0v2",
            ],
            biaya: ["M8 12h8"],
            refresh: ["M21 12a9 9 0 1 1-2.6-6.4", "M21 3v4h-4"],
            scan: [
                "M3 7V5a2 2 0 0 1 2-2h2M17 3h2a2 2 0 0 1 2 2v2M21 17v2a2 2 0 0 1-2 2h-2M7 21H5a2 2 0 0 1-2-2v-2M6 12h12",
            ],
            warn: [
                "M12 9v4M12 17h.01",
        ...
        let body = (P[name] || [])
            .map(
                (d) =>
                    `<path d="${d}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="stroke:${color};fill:none"></path>`,
            )
            .join("");
        if (circles[name]) {
            const c = circles[name];
            body += `<circle cx="${c[0]}" cy="${c[1]}" r="${c[2]}" stroke-width="1.8" style="stroke:${color};fill:none"></circle>`;
        }
    ...
    const A = (fn) => {
        reg.push(fn);
        return `data-a="${reg.length - 1}"`;
    }; // click handler
    const I = (fn) => {
        reg.push(fn);
        return `data-i="${reg.length - 1}"`;
    }; // input handler
    ...
    function flash(msg) {
        setState({ toast: msg });
        clearTimeout(toastT);
        toastT = setTimeout(() => setState({ toast: "" }), 2600);
    }

    function applyThemeBg(t) {
        const c = t === "light" ? "#ECEAE3" : "#0a0a0c";
    ...
    async function login() {
        ...
        if (!u) {
            setState({ loginErr: "Masukkan username terlebih dahulu." });
            return;
        }
        try {
    ...
    async function markPaid(id) {
        try {
            await api("/api/receivables/" + id + "/pay", "POST", {});
            await loadAll();
            flash("Tagihan ditandai lunas");
        } catch (e) {
            flash(e.message);
        }
    }
    ...
    function askConfirmDelete(label, onConfirm) {
        setState({ confirmDelete: { label, onConfirm } });
    }
    ...
    async function saveProduct() {
        if (!S.pName.trim()) {
            flash("Isi nama produk dulu");
            return;
        }
        const harga = parseInt(S.pHarga) || 0;
        if (!harga) {
            flash("Isi harga jual dulu");
            return;
        }
        if (!S.pBranch) {
            flash("Pilih cabang dulu");
            return;
        }
        ...
        try {
            await api("/api/products", "POST", {
                ...
                varian: S.pVar.trim() || "-",
                harga,
                modal: parseInt(S.pModal) || 0,
                stok: parseInt(S.pStok) || 0,
                kategori: S.pKat,
                branch: S.pBranch,
                barcode: S.pBarcode.trim() || null,
                exp: S.pExp || null,
            });
    ...
    async function saveBranch() {
        ...
        if (!name) {
            flash("Isi nama cabang terlebih dahulu");
            return;
        }
        try {
    ...
    async function saveCategory() {
        ...
        if (!name) {
            flash("Isi nama kategori terlebih dahulu");
            return;
        }
        try {
    ...
    async function saveBxCategory() {
        ...
        try {
            if (edit)
                await api("/api/expense-categories/" + S.editBxCatId, "PATCH", { name });
    ...
    async function openMemberDetail(uname) {
        if (S.memberOpen === uname) {
            setState({ memberOpen: null });
            return;
        }
    ...
    async function saveUser() {
        ...
        if (!S.uName.trim() || !S.uUname.trim() || (!edit && !S.uPass)) {
            flash("Lengkapi nama, username, dan password");
            return;
        }
        const body = {
            name: S.uName.trim(),
            username: S.uUname.trim(),
            password: S.uPass || null,
            role: S.uRole,
            branch: S.uCabang,
        };
        try {
    ...
    async function saveSupplier() {
        if (!S.poName.trim()) {
            flash("Isi nama supplier dulu");
            return;
        }
        const amount = parseInt(S.poAmount) || 0;
        if (!amount) {
            flash("Isi nominal hutang dulu");
            return;
        }
        if (!S.poDue) {
            flash("Isi tanggal jatuh tempo dulu");
            return;
        }
        try {
    ...
    async function paySupplier(s) {
        try {
            await api("/api/suppliers/" + s.id + "/pay", "POST", {});
            await loadAll();
            flash("Hutang " + s.name + " ditandai lunas");
        } catch (e) {
            flash(e.message);
        }
    }
    ...
    async function savePromo() {
        if (!S.prName.trim()) {
            flash("Isi nama promo dulu");
            return;
        }
        if (!S.prValue.trim()) {
            flash("Isi nilai promo (mis. 15% / Hemat Rp40.000)");
            return;
        }
        try {
            await api("/api/promos", "POST", {
                name: S.prName.trim(),
                desc: S.prDesc.trim(),
                type: S.prType,
                value: S.prValue.trim(),
            });
    ...
    async function deletePromo(p) {
        try {
            await api("/api/promos/" + p.id, "DELETE");
            await loadAll();
            flash('Promo "' + p.name + '" dihapus');
        } catch (e) {
            flash(e.message);
        }
    }
    async function saveExpense() {
        ...
        if (!amount) {
            flash("Isi nominal dulu");
            return;
        }
        if (S.bxRecurring && !S.bxDueDay) {
            flash("Isi tanggal jatuh tempo tiap bulan");
            return;
        }
        if (!S.bxRecurring && !S.bxDate) {
            flash("Isi tanggal biaya ini terjadi");
            return;
        }
        const edit = S.editExpenseId !== null;
        const body = {
            category: S.bxCategory,
            note: S.bxNote.trim(),
            amount,
            branch: S.bxBranch,
            recurring: S.bxRecurring,
    ...
    function editExpense(e) {
        setState({
            biayaForm: true,
            editExpenseId: e.id,
            bxCategory: e.category,
            bxNote: e.note || "",
            bxAmount: String(e.amount),
            bxBranch: e.cabang,
            bxRecurring: e.recurring,
    ...
    async function payExpense(x) {
        try {
            await api("/api/expenses/" + x.id + "/pay", "POST", {});
            await loadAll();
            flash("Biaya " + x.category + " ditandai lunas");
        } catch (e) {
            flash(e.message);
        }
    }
   ...
   (public/js/vendor/zxing.min.js, global window.ZXing) dipakai kalau native tak ada. */
    ...
    function decodeFrameFromVideo(videoElement) {
        ...
        try {
            ...
            const source = new ZXing.HTMLCanvasElementLuminanceSource(
                scanCanvas,
            );
    ...
    async function startScan(target) {
        setState({
            ...
            scanManual: "",
            scanMsg: "",
            prodForm: target === "pbarcode" ? false : S.prodForm,
        });
        ...
        if (!hasNative && !hasZxing) {
            setState({
                scanMsg:
                    "Browser ini tidak mendukung deteksi otomatis — ketik nomor barcode di bawah.",
            });
            return;
        }
        if (hasNative) {
    ...
    async function openScanDevice(deviceId) {
        ...
        setState({
            scanDeviceId:
                scanStream.getVideoTracks()[0]?.getSettings().deviceId || null,
            scanMsg: "",
        });
        render();
    ...
    async function changeScanDevice(deviceId) {
        try {
            await openScanDevice(deviceId);
        } catch (e) {
            setState({ scanMsg: "Gagal ganti kamera — coba pilih lagi." });
        }
    }
    let scanBusy = false;
    ...
    function handleScanResult(code) {
        ...
        if (!p) {
            flash("Barcode " + code + " tidak cocok dengan produk mana pun");
            return;
        }
        stopScan();
    ...
    function stopScan() {
        ...
        setState({
            scan: false,
            prodForm: S.scanTarget === "pbarcode" ? true : S.prodForm,
        });
    }

    /* ---- scan kamera KASIR (stream & state terpisah dari scan admin) ---- */
    let kScanStream = null,
        kScanTimer = null,
        kScanDetector = null;
    let kLastCode = "",
        kLastAt = 0,
    ...
    async function startScanKasir(mode) {
        ...
        if (!hasNative && !hasZxing) {
            setState({
                k_scanMsg:
                    "Browser tidak mendukung deteksi otomatis — ketik nomor barcode di bawah.",
            });
            return;
        }
        if (hasNative) {
    ...
    async function openKScanDevice(deviceId) {
        ...
        setState({
            k_scanDeviceId:
                kScanStream.getVideoTracks()[0]?.getSettings().deviceId || null,
            k_scanMsg: "",
        });
        render();
    ...
    async function changeScanDeviceKasir(deviceId) {
        try {
            await openKScanDevice(deviceId);
        } catch (e) {
            setState({ k_scanMsg: "Gagal ganti kamera — coba pilih lagi." });
        }
    }
    ...
    function handleScanKasir(code) {
        ...
        document.dispatchEvent(
            new CustomEvent("k-scan-result", {
                detail: { code, mode: S.k_scanMode },
            }),
        );
    }
    function stopScanKasir() {
        ...
        kScanTimer = null;
        if (kScanStream) {
            kScanStream.getTracks().forEach((t) => t.stop());
            kScanStream = null;
        }
    ...
    async function saveRestock() {
        ...
        if (!qty) {
            flash("Isi jumlah stok yang ditambahkan");
            return;
        }
        try {
            await api("/api/products/" + S.restockId + "/restock", "POST", {
                qty,
            });
    ...
    async function toggleUser(u) {
        try {
            ...
            await loadAll();
            flash(
                r.active ? u.name + " diaktifkan" : u.name + " dinonaktifkan",
            );
        } catch (e) {
            flash(e.message);
        }
    ...
    function markPaidBtnHtml(r, isFullWidth = false) {
        if (r.paid) return ""; // Baris yang sudah lunas → sel aksi dibiarkan kosong
    ...
    function recvView() {
        return DB.receivables.map((r) => {
            ...
            if (r.paid) {
                ...
                status = dl === 0 ? "Jatuh Tempo Hari Ini" : "Tempo H-" + dl;
                color = "#FBBF24";
            ...
            return { ...r, dl, status, color, bg, border, soon: !r.paid && dl <= 3 };
        });
    }
    function expenseDueView() {
        return DB.expenses
            .filter((e) => e.recurring && !e.paid)
            .map((e) => {
    ...
    function getDash(b) {
        ...
        if (b === "Semua") {
            ...
            const maxWeek = list.reduce(
                (a, d) => (d.week.length > a.length ? d.week : a),
                [],
            );
            const week = maxWeek.map((w, i) => ({
                label: w.label,
                v: +list
                    .reduce((s, d) => s + ((d.week[i] || {}).v || 0), 0)
                    .toFixed(2),
            }));
            const tops = {};
            list.forEach((d) =>
                d.top.forEach((t) => {
                    tops[t.name] = (tops[t.name] || 0) + t.sold;
                }),
            );
            const top = Object.entries(tops)
    ...
    function getYearly(b, year) {
        ...
        if (b === "Semua") {
            ...
            const maxMonths = list.reduce(
                (a, d) => (d.months.length > a.length ? d.months : a),
                [],
            );
            const months = maxMonths.map((m, i) => ({
                label: m.label,
                v: +list
                    .reduce((s, d) => s + ((d.months[i] || {}).v || 0), 0)
                    .toFixed(2),
            }));
    ...
    function renderVals() {
        ...
        const recv = recvView();
        const recvBranch = recv.filter(
            (r) => branch === "Semua" || r.cabang === branch,
        );
        const expDueBranch = expenseDueView().filter(
            (e) => branch === "Semua" || e.cabang === branch,
        );
        const dueSoon = [
            ...recvBranch
                .filter((r) => r.soon)
                .map((r) => ({ ...r, kind: "piutang" })),
            ...expDueBranch
                .filter((e) => e.soon)
                .map((e) => ({ ...e, kind: "biaya" })),
        ];
        ...
        const weekBars = D.week.map((w, i) => {
            const today = i === D.week.length - 1;
            return {
                label: w.label,
                valText: w.v.toFixed(1),
                valColor: today ? "var(--gold)" : "var(--muted)",
                h: ((w.v / wmax) * 100).toFixed(0) + "%",
                fill: today
                    ? "linear-gradient(180deg,var(--goldhi),var(--gold))"
                    : "var(--barempty)",
            };
        });
        const tmax = Math.max(...D.top.map((t) => t.sold), 1);
        const topProducts = D.top.map((t) => ({
            name: t.name,
            soldText: t.sold + " terjual",
            w: ((t.sold / tmax) * 100).toFixed(0) + "%",
        }));
        const piutangTotal = recvBranch
            .filter((r) => !r.paid)
            .reduce((s, r) => s + r.amount, 0);
        const dueSoonTotal = dueSoonRecv.reduce((s, r) => s + r.amount, 0);

        const bellItems = dueSoon
            .slice()
            .sort((a, b) => a.dl - b.dl)
            .map((r) => {
                const over = r.dl < 0;
                return {
                    name: r.kind === "biaya" ? "Biaya: " + r.category : r.name,
                    amountText: rp(r.amount),
                    cabang: r.cabang,
                    dueText: over
                        ? "Lewat " + Math.abs(r.dl) + "h"
                        : r.dl === 0
                          ? "Hari ini"
                          : "H-" + r.dl,
                    dueColor: over ? "var(--danger)" : "var(--warn)",
                    dotBg: over ? "var(--dangertint)" : "var(--warntint)",
                    dotColor: over ? "var(--danger)" : "var(--warn)",
                };
            });
        ...
        const sectionTitles = {
            ...
            laporan: "Laporan Omset",
            produk: "Produk & Harga",
            supplier: "Pembelian / Supplier",
            promo: "Promo & Bundle",
            biaya: "Biaya Operasional",
            shopee: "Integrasi Shopee",
        };
        const adminSet = [
            "dashboard",
            "piutang",
        ...
        const sidebarItems = sbDef.map((d) => {
            ...
            return {
                ...
                icon: ic(
                    d.k,
                    on ? "var(--gold)" : "var(--muted2)",
                    d.nested ? 16 : 19,
                ),
                bg: on ? "var(--goldtint2)" : "transparent",
                cl: on ? "var(--gold)" : "var(--muted2)",
                bd: on ? "rgba(212,175,55,.4)" : "transparent",
                onClick: go(d.k),
            };
        });
        ...
        const moreActive =
            S.more ||
            bottomTabsMore.includes(S.screen) ||
            S.screen === "settings";
        ...
        const bottomNav = [
            {
                key: "dashboard",
                label: "Beranda",
                on: S.screen === "dashboard",
                onClick: go("dashboard"),
            },
            {
                key: "piutang",
                label: "Piutang",
                on: S.screen === "piutang" || S.screen === "tempo",
                onClick: go("piutang"),
            },
            {
                key: "stok",
        ...
        const moreItems = moreDef.map((d) => ({
            label: d.label,
            icon: ic(d.k, "var(--gold)", 21),
            onClick: go(d.k),
        }));

        const pq = S.pq.trim().toLowerCase();
        const piutangRows = recvBranch
            .filter((r) => {
                if (S.pf === "Belum Lunas" && (r.paid || r.dl < 0))
                    return false;
                if (S.pf === "Terlambat" && !(!r.paid && r.dl < 0))
                    return false;
            ...
            .map((r) => {
                ...
                return {
                    ...
                    notPaid: !r.paid,
                    onPaid,
                    statusBadgeHtml: receivableStatusBadge(r),
                    actionHtml: markPaidBtnHtml(item, false),
                    actionMobileHtml: markPaidBtnHtml(item, true),
                };
            });
        const pfChips = ["Semua", "Belum Lunas", "Terlambat", "Lunas"].map(
            (f) => ({
                label: f,
                ...chip(S.pf === f),
                onClick: () => setState({ pf: f }),
            }),
        );

        const tempoRows = recvBranch
            .filter((r) => !r.paid)
            .sort((a, b) => a.dl - b.dl)
            .map((r) => {
                const over = r.dl < 0;
                return {
                    ...
                    name: r.name,
                    amountText: rp(r.amount),
                    dueDateText: fmtDate(r.due),
                    badge: over
                        ? "Terlambat " + Math.abs(r.dl) + " hari"
                        : r.dl === 0
                    ...
                    color: over
                        ...
                        : r.dl <= 1
                          ? "var(--warn)"
                          : "var(--gold)",
                    bg: over ? "var(--dangertint)" : "var(--warntint)",
                    onPaid: () => askConfirmPay("receivable", r),
                };
            });

        const cats = ["Semua", ...DB.categories.map((c) => c.name)];
        const stokRows = DB.products
            .filter(
                (p) =>
                    (branch === "Semua" || p.cabang === branch) &&
                    (S.stokCat === "Semua" || p.kategori === S.stokCat),
            )
            .map((p) => {
                let st, c, bg;
                if (p.stok <= 0) {
        ...
        const catChips = cats.map((c) => ({
            label: c,
            ...chip(S.stokCat === c),
            onClick: () => setState({ stokCat: c }),
        }));

        const userRows = DB.users
            .filter(
                (u) =>
                    (branch === "Semua" || u.cabang === branch) &&
                    (S.userRole === "Semua" || u.role === S.userRole),
            )
            .map((u) => ({
                name: u.name,
                unameText: "@" + u.uname,
                role: u.role,
                roleColor: u.role === "Admin" ? "var(--gold)" : "var(--info)",
                roleBg:
                    u.role === "Admin" ? "var(--goldtint2)" : "var(--infotint)",
                cabang: u.cabang,
                statusText: u.active ? "Aktif" : "Nonaktif",
                statusColor: u.active ? "var(--ok)" : "var(--dim)",
        ...
        const uRoleChips = ["Semua", "Admin", "Kasir"].map((r) => ({
            label: r,
            ...chip(S.userRole === r),
            onClick: () => setState({ userRole: r }),
        }));

        const produkRows = DB.products
            .filter((p) => branch === "Semua" || p.cabang === branch)
            .map((p) => {
                const margin =
                    (((p.harga - p.modal) / p.harga) * 100).toFixed(0) + "%";
                let expB = null;
                if (p.exp && p.exp !== "-") {
                    const d = Math.round(
                        (new Date(p.exp + "-01") - TODAY) / 86400000,
                    );
                    if (d <= 30)
                        expB = {
                            t: "Kedaluwarsa <30 hari",
                            c: "var(--danger)",
                            bg: "var(--dangertint)",
                        };
                    else if (d <= 60)
                        expB = {
                            t: "Kedaluwarsa <60 hari",
                            c: "var(--warn)",
                            bg: "var(--warntint)",
                        };
                    else if (d <= 90)
                        expB = {
                            t: "Kedaluwarsa <90 hari",
                            c: "var(--gold)",
                            bg: "var(--goldtint)",
                        };
                }
                return {
                    ...
                    varian: p.varian,
                    kategori: p.kategori,
                    hargaText: rp(p.harga),
                    modalText: rp(p.modal),
                    margin,
                    hasExp: !!expB,
                    expText: expB ? expB.t : "",
                    expColor: expB ? expB.c : "var(--muted)",
                    expBg: expB ? expB.bg : "transparent",
        ...
        const lapMap = {
            Harian: {
                total: D.today,
                bars: D.week.map((w) => ({ label: w.label, v: w.v })),
            },
            Mingguan: {
                total: Math.round(D.month / 4),
                bars: [
                    { label: "Mg 1", v: ((D.month / 4) * 0.9) / 1e6 },
                    { label: "Mg 2", v: ((D.month / 4) * 1.05) / 1e6 },
                    { label: "Mg 3", v: ((D.month / 4) * 0.95) / 1e6 },
                    { label: "Mg 4", v: ((D.month / 4) * 1.1) / 1e6 },
                ],
            },
            Bulanan: {
                total: D.month,
                bars: (yCurrent?.months || []).slice(-6),
            }, // 6 bulan terakhir (data asli)
        ...
        const lapLoading =
            (S.period === "Bulanan"
                ...
                : S.period === "Tahunan"
                  ...
                  : {}) === null;
        const lmax = Math.max(
            ...(lapSel.bars.length ? lapSel.bars.map((b) => b.v) : [0]),
            0.1,
        );
        const lapBars = lapSel.bars.map((b, i) => ({
            label: b.label,
            valText: b.v.toFixed(1),
            h: ((b.v / lmax) * 100).toFixed(0) + "%",
            fill:
                i === lapSel.bars.length - 1
                    ? "linear-gradient(180deg,var(--goldhi),var(--gold))"
                    : "var(--barempty)",
            valColor:
                i === lapSel.bars.length - 1 ? "var(--gold)" : "var(--muted)",
        }));
        const lapTotal = lapSel.total;
        const lapMethods = [
            {
                label: "Tunai",
                amt: lapTotal * ratios.tunai,
                color: "var(--ok)",
            },
            {
                label: "Marketplace",
                amt: lapTotal * ratios.market,
                color: "var(--info)",
            },
            {
                label: "Tempo",
                amt: lapTotal * ratios.tempo,
                color: "var(--warn)",
            },
        ].map((m) => ({
            label: m.label,
            amountText: rp(m.amt),
            w: ((m.amt / (lapTotal || 1)) * 100).toFixed(0) + "%",
            color: m.color,
        }));
        const periodChips = ["Harian", "Mingguan", "Bulanan", "Tahunan"].map(
            (p) => ({
                label: p,
                ...chip(S.period === p),
        ...
        const yearMin = curYear - 5,
            yearMax = curYear;
        const onPrevYear =
            S.selYear > yearMin ? () => changeSelYear(S.selYear - 1) : null;
        const onNextYear =
            S.selYear < yearMax ? () => changeSelYear(S.selYear + 1) : null;
        const abList = allBranches();
        const bcMax = Math.max(...abList.map((b) => getDash(b).month), 1);
        const branchCompare = abList.map((b) => {
            const d = getDash(b);
            const on = branch === b || branch === "Semua";
            return {
                label: b,
                amountText: rp(d.month),
                w: ((d.month / bcMax) * 100).toFixed(1) + "%",
                fill: on
                    ? "linear-gradient(90deg,var(--gold),var(--goldhi))"
                    : "var(--barempty)",
            };
        });
        const pbMax = Math.max(...abList.map((b) => getDash(b).today), 1);
        const perBranchBars = abList.map((b) => {
            const d = getDash(b);
            return {
                label: b,
                amountText: rp(d.today),
                monthText: "Bulan ini " + rpShort(d.month),
                w: ((d.today / pbMax) * 100).toFixed(0) + "%",
                onClick: () => setState({ branch: b }),
            };
        });
        ...
        const mTotals = Object.fromEntries(
            (DB.byUser[S.uPeriod] || []).map((r) => [r.uname, r]),
        );
        const mRanked = DB.users
            .filter(
                (u) => u.active && (branch === "Semua" || u.cabang === branch),
            )
            .map((u) => ({
                name: u.name,
                uname: u.uname,
                role: u.role,
                cabang: u.cabang,
                total: +((mTotals[u.uname] || {}).total || 0),
                trx: +((mTotals[u.uname] || {}).trx || 0),
            }))
            .sort((a, b) => b.total - a.total);
        mRanked.forEach((u, i) => (u.rank = i + 1));
        const mMax = Math.max(...mRanked.map((u) => u.total), 1);
        const mSel = new Set(S.selMembers);
        const toggleSel = (uname) => {
            const n = new Set(S.selMembers);
            if (n.has(uname)) n.delete(uname);
            else n.add(uname);
            setState({ selMembers: [...n] });
        };
        const removeSel = (uname) => {
            const n = new Set(S.selMembers);
            n.delete(uname);
            setState({ selMembers: [...n] });
        };
        // tabel menampilkan yang dipilih; kalau belum ada yang dipilih → semua (peringkat penuh)
        const mVisible = mSel.size
            ? mRanked.filter((u) => mSel.has(u.uname))
            : mRanked;
        const mAllZero =
            mVisible.length > 0 && mVisible.every((u) => u.total === 0);
        const memberRows = mVisible.map((u) => ({
            ...
            roleText: u.role,
            cabang: u.cabang,
            totalText: rp(u.total),
            trxText: u.trx + "×",
            trxLong: u.trx + " transaksi",
            w: ((u.total / mMax) * 100).toFixed(0) + "%",
            hasSales: u.total > 0,
            open: S.memberOpen === u.uname,
            onDetail: () => openMemberDetail(u.uname),
        ...
        const mOptionsSource = mq
            ? mRanked.filter((u) =>
                  (u.name + " @" + u.uname).toLowerCase().includes(mq),
              )
            : mRanked;
        const memberOptions = mOptionsSource.map((u) => ({
            name: u.name,
            unameText: "@" + u.uname,
            roleText: u.role,
            cabang: u.cabang,
            totalText: u.total > 0 ? rp(u.total) : "",
            checked: mSel.has(u.uname),
            onClick: () => toggleSel(u.uname),
        }));
        ...
        const selectAllVisible = () =>
            setState({
                selMembers: [
                    ...new Set([
                        ...S.selMembers,
                        ...mOptionsSource.map((u) => u.uname),
                    ]),
                ],
            });
        const memberAllVisibleSelected =
            mOptionsSource.length > 0 &&
            mOptionsSource.every((u) => mSel.has(u.uname));
        // chip filter aktif: satu chip per pegawai terpilih, bisa dihapus satu-satu
        const memberSelChips = mRanked
            .filter((u) => mSel.has(u.uname))
            .map((u) => ({ name: u.name, onRemove: () => removeSel(u.uname) }));
        ...
        const uPeriodWider = { Mingguan: "Bulanan", Bulanan: "Tahunan" }[
            S.uPeriod
        ];

        const supplierTotal = DB.suppliers
            .filter((s) => !s.paid)
        ...
        const expenseRows = DB.expenses
            ...
            .map((e) => {
                ...
                return {
                    ...
                    recurringText: e.recurring
                        ? "Rutin · tgl " + e.dueDay
                        : "Sekali ini",
                    status,
                    color,
                    bg,
                    canPay: e.recurring && !e.paid,
                    onPay: () => payExpense(e),
                    onEdit: () => editExpense(e),
                };
            });
        const curMonth = TODAY.getMonth(),
            curYearNum = TODAY.getFullYear();
        const expenseMonthTotal = DB.expenses
            .filter((e) => branch === "Semua" || e.cabang === branch)
            .filter((e) => {
                const d = new Date(e.date);
                return (
                    d.getMonth() === curMonth && d.getFullYear() === curYearNum
                );
            })
        ...
        const restockProd =
            S.restockId !== null
                ? DB.products.find((p) => p.id === S.restockId)
                : null;
        ...
        return {
            ...
            secDashboard: S.screen === "dashboard",
            secPiutang: S.screen === "piutang",
            secTempo: S.screen === "tempo",
            secStok: S.screen === "stok",
            secUsers: S.screen === "users",
            secLaporan: S.screen === "laporan",
            secProduk: S.screen === "produk",
            secSupplier: S.screen === "supplier",
            secPromo: S.screen === "promo",
            secBiaya: S.screen === "biaya",
            secShopee: S.screen === "shopee",
            sectionTitle:
                (isMobile
                    ? sectionTitlesMobile[S.screen]
                    : sectionTitles[S.screen]) || "Dashboard",
            ...
            moreItems,
            moreOpen: S.more,
            moreActiveColor: bnColor(moreActive),
            openMore: () => setState({ more: true }),
            closeMore: () => setState({ more: false }),
            ...
            toggleSidebar: () =>
                setState((s) => ({ sideCollapsed: !s.sideCollapsed })),
            ...
            settingsBranchText:
                branch === "Semua" ? "Semua Cabang" : "Cabang " + branch,
            tempoScopeText:
                branch === "Semua" ? "semua cabang" : "cabang " + branch,
            ...
            backFromSettings: () =>
                setState({
                    screen:
                        S.settingsBack ||
                        (S.role === "admin" ? "dashboard" : "cashier"),
                }),
            ...
            bellCount,
            bellItems,
            toggleBell: () => setState({ bell: !S.bell }),
            closeBell: () => setState({ bell: false }),
            goTempoFromBell: () =>
                S.role === "admin"
                    ? setState({ bell: false, screen: "tempo" })
                    : setState({ bell: false }),

            d_todayText: rp(D.today),
            d_trendText: D.trend
                ? D.trend + " dari kemarin"
                : D.semua
                  ? "gabungan semua cabang"
                  : "Cabang baru — belum ada transaksi",
            d_tunaiText: rpShort(D.tunai),
            d_marketText: rpShort(D.market),
            d_tempoText: rpShort(D.tempo),
            d_monthText: rp(D.month),
            d_trxCount: D.trx + " transaksi",
            weekBars,
            topProducts,
            d_piutangText: rp(piutangTotal),
            d_dueSoonText:
                dueSoonRecv.length + " tagihan · " + rpShort(dueSoonTotal),

            navPiutang: go("piutang"),
            branchMenu: S.branchMenu,
            toggleBranchMenu: () =>
                setState({ branchMenu: !S.branchMenu, bell: false }),
            ...
            closeScan: () => stopScan(),
            scanMsg: S.scanMsg,
            scanDevices: S.scanDevices,
            scanDeviceId: S.scanDeviceId,
            onScanDevice: (e) => changeScanDevice(e.target.value),
            scanManual: S.scanManual,
            onScanManual: (e) => setState({ scanManual: e.target.value }),
            useManual: () => handleScanResult(S.scanManual),

            piutangRows,
            pfChips,
            pq: S.pq,
            onPQ: (e) => setState({ pq: e.target.value }),
            piutangEmpty: piutangRows.length === 0,
            tempoRows,
            tempoEmpty: tempoRows.length === 0,
            stokRows,
            catChips,
            openScanStok: () => startScan("stok"),
            stokEmpty: stokRows.length === 0,
            catForm: S.catForm,
            newCat: S.newCat,
            ...
            openUserForm: () =>
                setState({
                    ...
                    uCabang:
                        branch === "Semua"
                            ? allBranches()[0] || "Pleburan"
                            : branch,
                }),
            userFormIsEdit: S.editUserId !== null,
            uPassShow: S.uPassShow,
            toggleUPass: () => setState({ uPassShow: !S.uPassShow }),
            closeUserForm: () => setState({ userForm: false }),
            uName: S.uName,
            onUName: (e) => setState({ uName: e.target.value }),
            uUname: S.uUname,
            onUUname: (e) => setState({ uUname: e.target.value }),
            uPass: S.uPass,
            onUPass: (e) => setState({ uPass: e.target.value }),
            uRoleTiles: ["Admin", "Kasir"].map((r) => ({
                label: r,
                on: S.uRole === r,
                onClick: () => setState({ uRole: r }),
            })),
            uCabangTiles: allBranches().map((b) => ({
                label: b,
                on: S.uCabang === b,
                onClick: () => setState({ uCabang: b }),
            })),
            saveUser: () => saveUser(),
            produkRows,
            prodForm: S.prodForm,
            ...
            openProdForm: () =>
                setState({
                    ...
                    pExp: "",
                    pBranch:
                        branch === "Semua" ? allBranches()[0] || "" : branch,
                }),
            closeProdForm: () => setState({ prodForm: false }),
            pName: S.pName,
            onPName: (e) => setState({ pName: e.target.value }),
            pVar: S.pVar,
            onPVar: (e) => setState({ pVar: e.target.value }),
            pKat: S.pKat,
            onPKat: (e) => setState({ pKat: e.target.value }),
            pHargaText: S.pHarga,
            onPHarga: (e) =>
                setState({ pHarga: (e.target.value || "").replace(/\D/g, "") }),
            pModalText: S.pModal,
            onPModal: (e) =>
                setState({ pModal: (e.target.value || "").replace(/\D/g, "") }),
            pStok: S.pStok,
            onPStok: (e) =>
                setState({ pStok: (e.target.value || "").replace(/\D/g, "") }),
            pBarcode: S.pBarcode,
            onPBarcode: (e) => setState({ pBarcode: e.target.value }),
            pExp: S.pExp,
            onPExp: (e) => setState({ pExp: e.target.value }),
            pBranch: S.pBranch,
            onPBranch: (e) => setState({ pBranch: e.target.value }),
            openScan: () => startScan("pbarcode"), // tombol scan di form produk → isi kolom barcode
            saveProd: () => saveProduct(),
            lapBars,
            lapTotalText: rp(lapTotal),
            lapMethods,
            periodChips,
            branchCompare,
            period: S.period,
            lapLoading,
            selYearText: String(S.selYear),
            onPrevYear,
            onNextYear,
            uPeriod: S.uPeriod,
            uPeriodChips: ["Mingguan", "Bulanan", "Tahunan"].map((p) => ({
                label: p,
                ...chip(S.uPeriod === p),
                onClick: () => changeUPeriod(p),
            })),
            memberRows,
            memberLoading: mLoading,
            memberNoData: !mLoading && mRanked.length === 0,
            memberAllZero: !mLoading && mAllZero,
            memberWiderPeriodLabel: uPeriodWider,
            onMemberWiderPeriod: uPeriodWider
                ? () => changeUPeriod(uPeriodWider)
                : null,
            ...
            memberDdLabel: mSel.size
                ? mSel.size + " pegawai dipilih"
                : "Semua pegawai",
            toggleMemberDd: () =>
                setState({
                    memberDropdown: !S.memberDropdown,
            ...
            memberSearch: S.memberSearch,
            onMemberSearch: (e) => setState({ memberSearch: e.target.value }),
            memberOptions,
            memberOptionsEmpty: memberOptions.length === 0,
            memberSelCount: mSel.size,
            memberSelChips,
            memberToolbarCountText: mSel.size
                ? mSel.size + " dari " + mRanked.length + " pegawai"
                : mRanked.length + " pegawai",
            clearMemberSel: () => setState({ selMembers: [] }),
            selectAllVisible,
            memberAllVisibleSelected,
            memberTotalText: rp(memberTotal),
            memberTrxText: memberTrx + " transaksi",
            memberFooterLabel: mSel.size
                ? mSel.size + " pegawai terpilih"
                : mRanked.length + " pegawai",
            memberCountText: mRanked.length + " pegawai",
            supplierRows,
            supplierTotalText: rp(supplierTotal),
            poForm: S.poForm,
            newPO: () =>
                setState({ poForm: true, poName: "", poAmount: "", poDue: "" }),
            closePoForm: () => setState({ poForm: false }),
            poName: S.poName,
            onPoName: (e) => setState({ poName: e.target.value }),
            poAmountText: S.poAmount,
            onPoAmount: (e) =>
                setState({
                    poAmount: (e.target.value || "").replace(/\D/g, ""),
                }),
            poDue: S.poDue,
            onPoDue: (e) => setState({ poDue: e.target.value }),
            ...
            prName: S.prName,
            onPrName: (e) => setState({ prName: e.target.value }),
            prDesc: S.prDesc,
            onPrDesc: (e) => setState({ prDesc: e.target.value }),
            prValue: S.prValue,
            onPrValue: (e) => setState({ prValue: e.target.value }),
            prTypeTiles: ["Bundle", "Diskon"].map((t) => ({
                label: t,
                on: S.prType === t,
                onClick: () => setState({ prType: t }),
            })),
            savePromo: () => savePromo(),

            expenseRows,
            expenseMonthTotalText: rp(expenseMonthTotal),
            expenseEmpty: expenseRows.length === 0,
            ...
            newBiaya: () =>
                setState({
                    biayaForm: true,
                    editExpenseId: null,
                    bxCategory: (DB.expenseCategories[0] || {}).name || "",
                    bxNote: "",
                    bxAmount: "",
                    bxBranch:
                        branch === "Semua" ? allBranches()[0] || "" : branch,
            ...
            closeBiayaForm: () =>
                setState({ biayaForm: false, editExpenseId: null }),
            bxCategory: S.bxCategory,
            onBxCategory: (e) => setState({ bxCategory: e.target.value }),
            bxNote: S.bxNote,
            onBxNote: (e) => setState({ bxNote: e.target.value }),
            bxAmountText: S.bxAmount,
            onBxAmount: (e) =>
                setState({
                    bxAmount: (e.target.value || "").replace(/\D/g, ""),
                }),
            bxBranch: S.bxBranch,
            onBxBranch: (e) => setState({ bxBranch: e.target.value }),
            bxRecurring: S.bxRecurring,
            bxTypeTiles: ["Sekali Ini", "Rutin Bulanan"].map((t) => ({
                label: t,
                on: (t === "Rutin Bulanan") === S.bxRecurring,
                onClick: () => setState({ bxRecurring: t === "Rutin Bulanan" }),
            })),
            bxDueDay: S.bxDueDay,
            onBxDueDay: (e) =>
                setState({
                    bxDueDay: (e.target.value || "").replace(/\D/g, ""),
                }),
            bxDate: S.bxDate,
            onBxDate: (e) => setState({ bxDate: e.target.value }),
            bxCategoryOptions: DB.expenseCategories.map((c) => c.name),
            confirmDeleteExpense: () =>
                askConfirmDelete(
                    'biaya "' + S.bxCategory + '" (' + rp(parseInt(S.bxAmount) || 0) + ")",
                    () => deleteExpense({ id: S.editExpenseId }),
                ),
            bxCatForm: S.bxCatForm,
            bxCatFormIsEdit: S.editBxCatId !== null,
            newBxCat: S.newBxCat,
            openBxCatForm: () =>
                setState({ bxCatForm: true, editBxCatId: null, newBxCat: "" }),
            closeBxCatForm: () =>
                setState({ bxCatForm: false, editBxCatId: null }),
            onNewBxCat: (e) => setState({ newBxCat: e.target.value }),
            saveBxCategory: () => saveBxCategory(),
            cancelEditBxCat: () =>
                setState({ editBxCatId: null, newBxCat: "" }),
            confirmDeleteBxCat: () => {
                const c = DB.expenseCategories.find(
                    (x) => x.id === S.editBxCatId,
                );
                if (c)
                    askConfirmDelete('kategori "' + c.name + '"', () =>
                        deleteBxCategory(c),
                    );
            },
            bxCatRows: DB.expenseCategories.map((c) => ({
                id: c.id,
                name: c.name,
                onEdit: () => editBxCategory(c),
            })),
            saveExpense: () => saveExpense(),

            restockOpen: restockProd !== null,
            restockName: restockProd
                ? restockProd.name + " · " + restockProd.varian
                : "",
            restockStokText: restockProd ? restockProd.stok + " pcs" : "",
            restockQty: S.restockQty,
            onRestockQty: (e) =>
                setState({
                    restockQty: (e.target.value || "").replace(/\D/g, ""),
                }),
    ...
    /* ================= shared svg snippets ================= */
    const svgBrand = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M4 9v6M20 9v6M7 7v10M17 7v10M7 12h10" stroke="#D4AF37" stroke-width="2" stroke-linecap="round"></path></svg>`;
    const svgDumb = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M5 9v6M19 9v6M8 7.5v9M16 7.5v9M8 12h8" stroke="#D4AF37" stroke-width="2" stroke-linecap="round"></path></svg>`;
    const svgSun = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="4.5" stroke="#D4AF37" stroke-width="1.8"></circle><path d="M12 2v2M12 20v2M2 12h2M20 12h2M5 5l1.5 1.5M17.5 17.5 19 19M19 5l-1.5 1.5M6.5 17.5 5 19" stroke="#D4AF37" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
    const svgMoon = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8Z" stroke="#D4AF37" stroke-width="1.8" stroke-linejoin="round"></path></svg>`;
    const svgBellIc = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M6 9a6 6 0 0 1 12 0c0 5 2 6 2 6H4s2-1 2-6Z" stroke="#D4AF37" stroke-width="1.7" stroke-linejoin="round"></path><path d="M10 20a2 2 0 0 0 4 0" stroke="#D4AF37" stroke-width="1.7" stroke-linecap="round"></path></svg>`;
    const svgGear = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="3" stroke="#D4AF37" stroke-width="1.7"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1Z" stroke="#D4AF37" stroke-width="1.7"></path></svg>`;
    const svgScanIc = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M3 7V5a2 2 0 0 1 2-2h2M17 3h2a2 2 0 0 1 2 2v2M21 17v2a2 2 0 0 1-2 2h-2M7 21H5a2 2 0 0 1-2-2v-2M6 12h12" stroke="#D4AF37" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
    const svgSearchIc = (w, left) =>
        `<svg style="position:absolute;left:${left}px;" width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="6.5" stroke="#6c6c74" stroke-width="1.8"></circle><path d="M16 16l4 4" stroke="#6c6c74" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
    const svgCartIc = (w) =>
        `<svg width="${w}" height="${w}" viewBox="0 0 24 24" fill="none"><path d="M3 4h2l2.4 12.2a1 1 0 0 0 1 .8h8.7a1 1 0 0 0 1-.8L21 8H6" stroke="#D4AF37" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"></path><circle cx="9" cy="20" r="1.4" fill="#D4AF37"></circle><circle cx="18" cy="20" r="1.4" fill="#D4AF37"></circle></svg>`;
    const themeBtn = (V, box, icon, rad) =>
        `<button ${A(V.toggleTheme)} style="width:${box}px;height:${box}px;border-radius:${rad}px;background:var(--surface2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;cursor:pointer;flex:none;">${V.isLight ? svgSun(icon) : svgMoon(icon)}</button>`;
    const lbl = (t) =>
        `<label style="display:block;font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);font-family:'Saira',sans-serif;font-weight:600;margin-bottom:6px;">${t}</label>`;
    const inputStyle = (h) =>
        `width:100%;box-sizing:border-box;height:${h}px;border-radius:12px;border:1px solid var(--border);background:var(--input);color:var(--text);font-size:14px;padding:0 12px;outline:none;font-family:'Hanken Grotesk',sans-serif;`;

    /* ================= custom form controls (Select, DatePicker, MonthPicker) ================= */
    function openCustomDD(
        id,
        value,
        options,
        onChange,
        ...
        const curIdx = normOpts.findIndex(
            (o) => String(o.value) === String(value),
        );
        setState({
            activeDD:
                S.activeDD && S.activeDD.id === id
                    ? null
                    : {
                          id,
                          value: String(value || ""),
            ...
            activeDP:
                S.activeDP && S.activeDP.id === id
                    ...
                    : {
                          ...
                          onChange,
                          placeholder,
                          viewYear: vDate.getFullYear(),
                          viewMonth: vDate.getMonth(),
                          styleHeight,
                      },
            activeDD: null,
            activeMP: null,
    ...
    function navCustomMPYear(delta) {
        if (!S.activeMP) return;
        setState({
            activeMP: { ...S.activeMP, viewYear: S.activeMP.viewYear + delta },
        });
    }
    ...
    if (typeof window !== "undefined" && !window._ss_pickers_bound) {
        ...
        document.addEventListener("keydown", function (e) {
            ...
            if (S.activeDD) {
                const dd = S.activeDD;
                const filtered = dd.options.filter(
                    (o) =>
                        !dd.search ||
                        o.label.toLowerCase().includes(dd.search.toLowerCase()),
                );
                if (e.key === "ArrowDown") {
                    e.preventDefault();
                    ...
                    const matchIdx = filtered.findIndex((o) =>
    ...
    function customMonthPickerHtml(
        id,
        value,
        onChange,
        placeholder = "Pilih bulan...",
  ...
  <div id="custom-portal-panel" class="scrl" style="position:fixed;background:#141416;border:1px solid rgba(255,255,255,0.14);border-radius:14px;padding:8px;box-shadow:0 24px 60px rgba(0,0,0,0.85);max-height:240px;overflow-y:auto;color:#F4F3EE;font-family:'Hanken Grotesk',sans-serif;box-sizing:border-box;">
      ...
      <div role="listbox" style="display:flex;flex-direction:column;gap:2px;">
        ${filtered
            .map((o, idx) => {
                ...
                let style =
                    "padding:9px 12px;border-radius:8px;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:space-between;transition:background .1s ease;";
                if (isSel) {
                    style +=
                        "background:rgba(212,175,55,0.15);color:#D4AF37;font-weight:700;";
                } else if (isAct) {
                    style +=
    ...
    function customDPPanelHtml(V) {
        ...
        for (let d = 1; d <= totalDays; d++) {
            ...
            let style =
                "height:32px;border-radius:8px;border:none;font-size:12.5px;font-family:'Hanken Grotesk',sans-serif;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .12s ease;";
            if (isSelected) {
                style +=
                    "background:#D4AF37;color:#0A0A0C;font-weight:800;box-shadow:0 0 10px rgba(212,175,55,0.4);";
            } else if (isToday) {
                style +=
            ...
            daysHtml.push(
                `<button ${A(() => selectCustomDPDate(dStr))} class="fx-hover" style="${style}">${d}</button>`,
            );
        }
  ...
  <div id="custom-portal-panel" class="scrl" style="position:fixed;background:#141416;border:1px solid rgba(255,255,255,0.14);border-radius:16px;padding:16px;box-shadow:0 24px 60px rgba(0,0,0,0.85);color:#F4F3EE;font-family:'Hanken Grotesk',sans-serif;width:300px;box-sizing:border-box;">
    ...
    function customMPPanelHtml(V) {
        ...
        let monthsHtml = FULL_MON.map((mName, idx) => {
            ...
            let style =
                "height:40px;border-radius:10px;border:none;font-size:12.5px;font-family:'Hanken Grotesk',sans-serif;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .12s ease;";
            if (isSelected) {
                style +=
                    "background:#D4AF37;color:#0A0A0C;font-weight:800;box-shadow:0 0 10px rgba(212,175,55,0.4);";
            } else {
  ...
  <div class="scrl" style="height:100dvh;overflow-y:auto;display:flex;flex-direction:column;padding:40px;background:radial-gradient(90% 70% at 50% 0%, var(--goldtint), transparent 55%);">
    ...
    /* ================= ADMIN sections ================= */
    const badge = (color, bg, text, fs) =>
        `<span style="font-size:${fs || 11}px;font-weight:700;padding:4px 9px;border-radius:7px;color:${color};background:${bg};">${text}</span>`;
    const chevronIc = (open, size) =>
        `<svg width="${size || 11}" height="${size || 11}" viewBox="0 0 24 24" fill="none" style="display:inline-block;vertical-align:middle;flex:none;transition:transform .15s ease;transform:rotate(${open ? 90 : 0}deg);"><path d="M9 6l6 6-6 6" style="stroke:var(--muted);fill:none" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
    ...
    function memberDetailHtml(m) {
        if (m.items === undefined)
            return `<div style="margin-top:10px;font-size:12.5px;color:var(--muted);">Memuat rincian produk…</div>`;
        if (m.items.length === 0)
            return `<div style="margin-top:10px;font-size:12.5px;color:var(--dim2);">Tidak ada penjualan produk pada periode ini.</div>`;
    ...
    <div style="background:var(--surface);border:1px solid var(--border2);box-shadow:var(--cardshadow);border-radius:18px;padding:20px;margin-top:16px;">
      ...
      ${
          V.memberLoading
              ? `<div style="border:1px dashed var(--border);border-radius:14px;padding:26px;text-align:center;color:var(--muted);font-size:13px;">Memuat penjualan anggota…</div>`
              : V.memberNoData
                ? `<div style="border:1px dashed var(--border);border-radius:14px;padding:26px;text-align:center;color:var(--dim2);font-size:13px;">Belum ada anggota aktif di cabang ini.</div>`
                : V.memberAllZero
                  ? `
    ...
    ${
        V.expenseEmpty
            ? `<div style="padding:40px 20px;text-align:center;color:var(--muted);font-size:13.5px;">Belum ada biaya tercatat.</div>`
            : V.isDesktop
              ? `
      <div style="background:var(--surface);border:1px solid var(--border2);box-shadow:var(--cardshadow);border-radius:16px;overflow:hidden;">
        ...
        ${V.expenseRows
          ...
          <div style="display:grid;grid-template-columns:1.4fr 1.6fr 1.2fr 1fr 1.6fr;padding:15px 18px;border-bottom:1px solid var(--divider);align-items:center;font-size:13.5px;">
            ...
            <span style="text-align:right;display:flex;gap:8px;justify-content:flex-end;align-items:center;">
              ...
              <button ${A(x.onEdit)} title="edit-biaya-${x.id}" style="height:32px;padding:0 13px;border-radius:9px;background:var(--chip);border:1px solid var(--border);color:var(--text2);font-size:12px;cursor:pointer;font-family:'Hanken Grotesk',sans-serif;">Edit</button>
            </span>
          </div>`,
            )
            .join("")}
          ...
          <div style="background:var(--surface);border:1px solid var(--border2);box-shadow:var(--cardshadow);border-radius:14px;padding:13px 15px;display:flex;justify-content:space-between;align-items:center;gap:10px;">
            ...
            <div style="text-align:right;flex:none;">
              ...
              <div style="margin-top:7px;display:flex;gap:6px;justify-content:flex-end;">
                ...
                <button ${A(x.onEdit)} title="edit-biaya-${x.id}" style="height:30px;padding:0 12px;border-radius:8px;background:var(--chip);border:1px solid var(--border);color:var(--text2);font-size:11.5px;cursor:pointer;font-family:'Hanken Grotesk',sans-serif;">Edit</button>
              </div>
            </div>
          </div>`,
            )
    ...
    function adminHtml(V) {
        const sections = {
            dashboard: secDashboardHtml,
            piutang: secPiutangHtml,
            tempo: secTempoHtml,
            stok: secStokHtml,
            users: secUsersHtml,
            laporan: secLaporanHtml,
            produk: secProdukHtml,
            supplier: secSupplierHtml,
            promo: secPromoHtml,
            biaya: secBiayaHtml,
            shopee: secShopeeHtml,
        };
  ...
  <div id="memberdd-panel" role="listbox" aria-label="Pilih pegawai" style="position:fixed;top:0;left:0;z-index:50;min-width:320px;background:var(--surface2);border:1px solid var(--border);border-radius:14px;overflow:hidden;box-shadow:0 18px 40px -12px var(--shadowc);${V.pop("memberDd")}">
    ...
    <div class="scrl" style="max-height:280px;overflow-y:auto;">
      ${
          V.memberOptionsEmpty
              ? `<div style="padding:16px;text-align:center;color:var(--dim2);font-size:12.5px;">Tidak ada pegawai cocok.</div>`
              : V.memberOptions
                    .map(
                        (o) => `
        ...
        ${
          ...
          <div style="width:140px;">
            ${customSelectHtml(
                "scandevice",
                V.scanDeviceId,
                V.scanDevices.map((d, i) => ({
                    value: d.deviceId,
                    label: d.label || "Kamera " + (i + 1),
                })),
                (v) => changeScanDevice(v),
                "Pilih kamera...",
                34,
            )}
          </div>
  ...
  <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:51;width:${modalW};background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:${pad};${V.popModal("branchForm")}">
    ...
    function userFormHtml(V) {
        ...
        const selTile = (t) =>
            `<button ${A(t.onClick)} style="flex:1;min-width:100px;height:44px;border-radius:11px;cursor:pointer;border:1px solid ${t.on ? "var(--gold)" : "var(--border)"};background:${t.on ? "var(--goldtint2)" : "var(--surface2)"};color:${t.on ? "var(--gold)" : "var(--muted)"};display:flex;align-items:center;justify-content:center;font-weight:600;font-size:13.5px;font-family:'Hanken Grotesk',sans-serif;">${esc(t.label)}</button>`;
        return `
  ...
  <div style="position:fixed;bottom:32px;left:50%;transform:translateX(-50%);z-index:60;background:var(--chip);border:1px solid var(--goldborder);border-radius:14px;padding:14px 22px;display:flex;align-items:center;gap:11px;box-shadow:0 16px 36px -10px var(--shadowc);${V.popToast}white-space:nowrap;">
    ...
    function render() {
        ...
        const openNow = {
            ...
            biayaForm: S.biayaForm,
            bxCatForm: S.bxCatForm,
            restock: S.restockId !== null,
            branchMenu: S.branchMenu,
            memberDd: S.memberDropdown,
            confirmPay: !!S.confirmPay,
            confirmDelete: !!S.confirmDelete,
            toast: !!S.toast,
        };
        ...
        const scrolls = sameScreen
            ? [...root.querySelectorAll(".scrl")].map((el) => el.scrollTop)
            : null;
        root.innerHTML = html(V);
        if (scrolls) {
            [...root.querySelectorAll(".scrl")].forEach((el, i) => {
                if (i < scrolls.length && scrolls[i]) el.scrollTop = scrolls[i];
            });
        }
        lastScreen = S.screen;
        prevOpen = openNow;
        ...
        if (S.k_scanMode && kScanStream) {
            ...
            if (kv && kv.srcObject !== kScanStream) {
                kv.srcObject = kScanStream;
                kv.play().catch(() => {});
            }
        }
        ...
        if (activePicker) {
            ...
            if (trig && panel) {
                ...
                const flipUp =
                    spaceBelow < pHeight + 12 && r.top > pHeight + 12;
                ...
                if (panelLeft + targetWidth > window.innerWidth - 12) {
                    panelLeft = Math.max(
                        12,
                        window.innerWidth - targetWidth - 12,
                    );
                }
    ...
    root.addEventListener("keydown", (e) => {
        if (
            e.key === "Enter" &&
            (e.target.id === "i-uname" || e.target.id === "i-pass")
        )
            login();
        if (e.key === "Escape" && S.memberDropdown)
            setState({ memberDropdown: false });
    });
   ...
   Detail lengkap ada di header public/js/kasir.js. */
    ...
    function renderCashier(V) {
        return cashierRenderer ? cashierRenderer(V) : cashierPlaceholder(V);
    }
    function cashierPlaceholder(V) {
        const btn = (handler, label, gold) =>
            `<button ${A(handler)} style="height:44px;padding:0 18px;border-radius:12px;cursor:pointer;font-family:'Hanken Grotesk',sans-serif;font-size:13.5px;font-weight:600;border:1px solid ${gold ? "var(--goldborder)" : "var(--border)"};background:${gold ? "var(--goldtint)" : "var(--surface2)"};color:${gold ? "var(--gold)" : "var(--text2)"};">${label}</button>`;
    ...
    window.SS = {
        ...
        startScanKasir,
        stopScanKasir,
        changeScanDeviceKasir,
        saveRestockKasir,
        registerCashier(fn) {
            cashierRenderer = fn;
            if (S.screen === "cashier") render();
        },
    };
    ...
    window.addEventListener("resize", () => {
        clearTimeout(rzT);
        rzT = setTimeout(() => setState({ vw: window.innerWidth }), 100);
    });
